...
const styles = require('./scss/Refresh.scss');
/**
 * <Refresh
...
export default class Refresh extends Component {
  static propTypes = {
    ...
    onSlideRangeEnd: PropTypes.func,                                        // 左右滑动距离
    onSlideRangeBegin: PropTypes.func,                                        // 左右滑动距离
  };
  ...
  constructor(props) {
    ...
    this.state = {
      IsShowRefresh: !RefreshComplete, IsShowLoadMore: !NextDataComplete,
      ScrollId: Utility.constItem.Events.OnScrollEvent + '_' + new Date().getTime()
    };
  }

  componentDidMount() {
    this.__InitJudge();
    this.state.StartTop = window.document.body.scrollTop;
    Utility.$on(this.state.ScrollId, this.__EventListenerScroll);
  }
  ...
  __EventListenerScroll() {
    // const { } = event;
    console.log('-----------------refresh-----------');
    const top = window.document.body.scrollTop;
    const startTope = this.state.StartTop;
    console.log(top, startTope, top - startTope);
  }
   ...
   */
  __HandlerRefresh() {
    // const GoComRefresh = this.refs.GoComRefresh;
    ...
    // const __differenceValue = bodyHeight - GoComRefresh.scrollHeight;
    const __bodyScrollTop = window.document.body.children[0].firstElementChild.scrollTop;// body.scrollTop || document.documentElement.scrollTop || window.pageYOffset;
    // if (GoComRefresh.scrollTop > 0) {
    if (__bodyScrollTop > 10) {
      return;
    }
    // }
  ...
  __HandlerNextData() {
    ...
    const __differValue = ((__bodyScrollHeight - __bodyScrollTop - window.screen.height) / __bodyScrollHeight) * 100;
  ...
  render() {
    const { LoadType, RefreshComplete, NextDataComplete } = this.props;
    return (
      <div ref="GoComRefresh" className={styles.refreshCss} style={this.__GetStyle()}
